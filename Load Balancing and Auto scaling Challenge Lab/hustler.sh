...
curl -X POST -H "Content-Type: application/json" \
  -H "Authorization: Bearer $token" \
  -d '{
    "name": "hustler-target-proxy",
    "urlMap": "projects/'"$project_id"'/global/urlMaps/hustler"
  }' \
...
curl -X POST -H "Content-Type: application/json" \
  -H "Authorization: Bearer $token" \
  -d '{
    "name": "hustler-target-proxy-2",
    "urlMap": "projects/'"$project_id"'/global/urlMaps/hustler"
  }' \
  "https://compute.googleapis.com/compute/v1/projects/$project_id/global/targetHttpProxies"


LB_IP_ADDRESS=$(gcloud compute forwarding-rules describe hustler --global --format="value(IPAddress)")
