...
IMAGE=$(gcloud artifacts docker images list $REGION-docker.pkg.dev/$PROJECT_ID/gcf-artifacts \
  --format="value(IMAGE)" | head -n 1)


gcloud run deploy temperature-converter \
--image=$IMAGE \
--set-env-vars=LOG_EXECUTION_ID=true,TEMP_CONVERT_TO=ctof \
--region=$REGION \
--project=$PROJECT_ID \
